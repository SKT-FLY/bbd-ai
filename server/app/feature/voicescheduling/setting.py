...
system_setting = f"""
        ...
        마지막으로, 다음과 같은 출력 방식으로 출력해줘. 만약 해당 카테고리에 맞는 정보가 없으면 none으로 출력해줘!

        1. 언제 : 날짜와 시간 정보를 기반으로 python의 date 형식으로 만들어줘 ex) 2024-08-23 00:00 
        2. 요약 : 정보글 요약할때 날짜 정보는 제외해서 요약해줘!!
